...
async function devDetail() {
    ...
    switch (dev) {
        case 'push':
            const branch = execSync('git fetch --all && git branch -r').toString().trim().split('\n');
            branch.shift();
            let hasil = []
            branch.forEach(br => {
                if (br.includes('origin/')) {
                    br = br.replace('origin/', '');
                    br = br.replace('\n', '').trim();
                    hasil.push(br);
                }
            });
            ...
            let pilihBranch = await new Select({
                name: "branch",
                message: "Pilih branch",
                choices: hasil,
                hint: "ayao dipilih dulu branchnya"
            }).run().catch(err => null);
            ...
            execSync(`git add . && git commit -m "$(date)" && git push origin ${pilihBranch}`, { stdio: 'inherit' });
